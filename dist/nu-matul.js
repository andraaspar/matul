...
__webpack_require__.d(__webpack_exports__, {
  ...
  "HtmlResultHandler": () => (/* reexport */ HtmlResultHandler_namespaceObject),
...
class HtmlResultHandler {
    ...
    add(p) {
        ...
        let result;
        const doc = p.parent.ownerDocument ?? p.parent;
    ...
    update(p) {
        ...
        if (p.virtual instanceof VElement) {
            for (let name of Object.keys(p.oldVirtual.props)) {
                if (!(name in p.virtual.props)) {
                    if (name === "class") {
                        ...
                        delete p.result.dataset[dataNameToDatasetName(name)];
                    }
                    else {
                        p.result[name] = "";
                    }
...
function dataNameToDatasetName(s) {
    return s.slice(5).replace(/-([a-z])/gi, (_, c) => c.toUpperCase());
}
const handlers = [];
...
function render() {
    if (renderQueued) {
        ...
        return (renderQueued = new Promise((resolve, reject) => {
...
;// CONCATENATED MODULE: ./src/nu-matul.ts
